...
use windows_sys::Win32::{
    ...
    UI::{
        ...
        WindowsAndMessaging::{
            FindWindowExW, FindWindowW, GetClientRect, GetParent, GetWindowLongPtrW,
            GetWindowRect, SetParent, SetWindowLongPtrW, SetWindowPos, ShowWindow,
            GWL_EXSTYLE, GWL_STYLE, HWND_TOP,
            SWP_FRAMECHANGED, SWP_NOACTIVATE, SWP_SHOWWINDOW, SW_SHOWNOACTIVATE, WS_CAPTION,
            WS_CHILD, WS_CLIPCHILDREN, WS_CLIPSIBLINGS, WS_EX_APPWINDOW, WS_EX_TOOLWINDOW,
...
pub fn attach(window: &WebviewWindow, position: WidgetPosition) -> Result<(), String> {
    ...
    unsafe {
        ...
        let widget_width = logical_to_physical_width(FALLBACK_LOGICAL_WIDTH, dpi)
            .min(taskbar_width);
        ...
        if previous_parent.is_null() && parent_error != 0 {
            ...
            return Err(format!("无法挂载到 Win11 任务栏，Win32 错误码：{parent_error}"));
        }
...
pub fn set_width(
    ...
    unsafe {
        let taskbar_hwnd = GetParent(widget_hwnd);
        if taskbar_hwnd.is_null() {
            return Err("组件尚未挂载到任务栏".to_string());
        }
...
fn resolve_widget_x(
    ...
    unsafe {
        ...
        if GetWindowRect(tray_hwnd, &mut tray_rect) == 0 {
            eprintln!("无法读取任务栏通知区域尺寸，组件将回退到靠左位置：{}", last_error("GetWindowRect 调用失败"));
            return 0;
        }
        ...
        if ScreenToClient(taskbar_hwnd, &mut tray_left) == 0 {
            ...
            eprintln!("无法将任务栏通知区域坐标转换到客户区，组件将回退到靠左位置，Win32 错误码：{error}");
            return 0;
        }
